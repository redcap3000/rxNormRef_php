...
// would like to some day implement this in SOAP...
   ...
   if($direct_load){
	// this is an example use as listed on the website .. only loads if loaded directly...

	    $rxTerms = new rxTermsApi(null,'json');
		print_r(json_decode($rxTerms->getAllRxTermInfo('198440')));
		print_r(json_decode($rxTerms->getRxTermDisplayName('198440')));
		print_r(json_decode($rxTerms->getRxTermsVersion()));
	}
